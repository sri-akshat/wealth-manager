# services/investment-service/tests/conftest.py
import pytest
import os
...
from test_utils.fixtures import create_test_client, create_test_token
...
def db_engine():
    ...
    with engine.connect() as conn:
        # Create indexes
        conn.execute(text("CREATE UNIQUE INDEX IF NOT EXISTS ix_mutual_funds_scheme_code ON mutual_funds (scheme_code)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS ix_investments_user_id ON investments (user_id)"))
        conn.commit()
...
def test_user_token():
    ...
    return create_test_token(settings.JWT_SECRET_KEY, "test@example.com")
